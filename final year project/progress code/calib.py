...
def check_calibration(camera0_name, camera0_data, camera1_name, camera1_data, _zshift = 50.):
    ...
    while True:
        ...
        if k == 27:
            ...
            _zshift = int(input("enter new Z value: "))
            z_shift = np.array([0., 0., _zshift]).reshape((1, 3))
            draw_axes_points = 5 * coordinate_points + z_shift
            pixel_points_camera0 = []
            pixel_points_camera1 = []
...
def run():
    ...
    R1, T1 = stereo_calibrate(cmtx0, dist0, cmtx1, dist1, frames_prefix_c0, frames_prefix_c1)
    ...
    P0 = get_projection_matrix(cmtx0, R_W0, T_W0)
    P1 = get_projection_matrix(cmtx1, R_W1, T_W1)
